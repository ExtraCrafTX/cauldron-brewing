...
import io.github.indicode.fabric.cauldronbrew.*;
...
import io.github.indicode.fabric.cauldronbrew.recipe.CauldronRecipe;
...
public class BrewingCauldronBlockEntity extends BlockEntity implements Tickable, BlockEntityClientSerializable {
    ...
    public boolean activate(BlockState blockState_1, World world_1, BlockPos blockPos_1, PlayerEntity playerEntity_1, Hand hand_1, BlockHitResult blockHitResult_1) {
        ...
        if (itemStack_1.isEmpty()) {
            ...
            if (item_1 == Items.WATER_BUCKET) {
                if (level < 3 && isFluidWater() && !world_1.isClient) {
    ...
    @Override
    public void tick() {
        List<Entity> items = this.world.getEntities(EntityType.ITEM, new Box(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), this.getPos().getX() + 1, this.getPos().getY() + 1, this.getPos().getZ() + 1), (Predicate<Entity>) input -> true);
    ...
    public void markDirty() {
        ...
        if (!world.isClient()){
            ...
            Object storage = manager.threadedAnvilChunkStorage;
            ChunkHolder holder = ((ITACSM)storage).getChunkHolders().get(chunk.getPos().toLong());
            holder. // im still working on it
        }
    }
    ...
    public BlockEntityUpdateS2CPacket toUpdatePacket() {
        return new BlockEntityUpdateS2CPacket(pos, 127, toTag(new CompoundTag()));
    }
